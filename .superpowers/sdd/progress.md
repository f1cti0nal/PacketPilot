...
- **tsc node types + ip_threats guard** (`parity.test.ts` + `package.json`): `@types/node` added as devDependency; bare `"fs"/"url"/"path"` imports switched to `node:` prefix; `enriched.summary.ip_threats` guarded with `?? []` (field is optional in `Summary` type). Approach: `@types/node` global — zero collateral tsc errors (no `setTimeout`/`Buffer`/`NodeJS.Timeout` conflicts in src). Verification: `npx tsc --noEmit` → exit 0, ZERO errors; parity test 1/1 pass; full suite 179/179 pass.
