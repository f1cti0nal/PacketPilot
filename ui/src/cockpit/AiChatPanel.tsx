...
import { BTN_GHOST_ICON, BTN_PRIMARY, INPUT_BASE } from "./primitives";
...
export function AiChatPanel({ open, onClose, output, model }: { open: boolean; onClose: () => void; output: AnalysisOutput; model: string }) {
  ...
  return (
    <>
      <div ref={ref} onKeyDown={onKeyDown} role="dialog" aria-modal="true" aria-label="AI chat" className="glass-band pp-slide-in-r fixed inset-y-0 right-0 z-50 flex w-[28rem] max-w-full flex-col border-l border-[var(--color-border)]" style={{ boxShadow: "var(--sh-float)" }}>
        <div className="flex items-center justify-between border-b border-[var(--color-border)] p-3">
          <h2 className="text-sm font-medium">Ask about this capture</h2>
          <button type="button" className={BTN_GHOST_ICON} aria-label="Close AI chat" onClick={onClose}>
            <X size={14} aria-hidden />
          </button>
        </div>
        <div role="log" aria-live="polite" aria-label="Conversation" className="flex-1 space-y-2 overflow-auto p-3 text-xs">
          {msgs.map((m, i) => (
            <div key={i} className={m.role === "user" ? "text-[var(--color-text)]" : "text-[var(--color-text-dim)]"}>
          ...
          {streaming && <div className="text-[var(--color-text-dim)]"><Markdown text={streaming} /></div>}
        </div>
        {error && (
        ...
        <div className="flex gap-2 border-t border-[var(--color-border)] p-3">
          <input className={`${INPUT_BASE} min-w-0 flex-1 px-2.5 py-1.5`} value={input}
          ...
          <button type="button" className={BTN_PRIMARY} onClick={() => send()} disabled={busy}>Send</button>
        </div>
      </div>
      {showConsent && (
        <AiConsent
