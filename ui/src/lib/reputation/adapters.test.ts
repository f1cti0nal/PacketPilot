...
describe("reputation adapters", () => {
  ...
  it("abuseipdb high confidence -> malicious", async () => {
    const v = await abuseipdbVerdict(fake(200, JSON.stringify({ data: { abuseConfidenceScore: 96, totalReports: 5 } })), "203.0.113.7", 1);
    expect(v.status).toBe("malicious");
    expect(v.score).toBe(96);
  });
  it("abuseipdb zero reports -> clean", async () => {
    const v = await abuseipdbVerdict(fake(200, JSON.stringify({ data: { abuseConfidenceScore: 0, totalReports: 0 } })), "1.2.3.4", 1);
    expect(v.status).toBe("clean");
  });
  it("abuseipdb medium confidence (25-74) -> unknown", async () => {
    const v = await abuseipdbVerdict(fake(200, JSON.stringify({ data: { abuseConfidenceScore: 40, totalReports: 3 } })), "1.2.3.4", 1);
    expect(v.status).toBe("unknown");
  });
  it("abuseipdb low score but has reports -> unknown", async () => {
    const v = await abuseipdbVerdict(fake(200, JSON.stringify({ data: { abuseConfidenceScore: 0, totalReports: 2 } })), "1.2.3.4", 1);
    expect(v.status).toBe("unknown");
  });
  it("abuseipdb non-200 status -> unavailable", async () => {
    const v = await abuseipdbVerdict(fake(500, "error"), "1.2.3.4", 1);
    expect(v.status).toBe("unavailable");
  });
  it("abuseipdb bad JSON -> unavailable", async () => {
    const v = await abuseipdbVerdict(fake(200, "not-json"), "1.2.3.4", 1);
    expect(v.status).toBe("unavailable");
  });
  ...
  it("greynoise benign -> benign + actor tag", async () => {
    const v = await greynoiseVerdict(fake(200, JSON.stringify({ classification: "benign", riot: false, noise: true, name: "Shodan.io" })), "203.0.113.7", 1);
    expect(v.status).toBe("benign");
    expect(v.tags).toContain("Shodan.io");
  });
  it("greynoise 404 -> notfound", async () => {
    const v = await greynoiseVerdict(fake(404, JSON.stringify({ message: "not observed" })), "203.0.113.7", 1);
    expect(v.status).toBe("notfound");
  });
  it("greynoise non-200/non-404 -> unavailable", async () => {
    const v = await greynoiseVerdict(fake(500, "server error"), "1.2.3.4", 1);
    expect(v.status).toBe("unavailable");
  });
  it("greynoise bad JSON -> unavailable", async () => {
    const v = await greynoiseVerdict(fake(200, "bad-json"), "1.2.3.4", 1);
    expect(v.status).toBe("unavailable");
  });
  it("greynoise malicious classification -> malicious", async () => {
    const v = await greynoiseVerdict(fake(200, JSON.stringify({ classification: "malicious", riot: false, noise: true })), "1.2.3.4", 1);
    expect(v.status).toBe("malicious");
    expect(v.score).toBe(95);
  });
  it("greynoise riot=true (no classification) -> benign + business-service tag", async () => {
    const v = await greynoiseVerdict(fake(200, JSON.stringify({ classification: "unknown", riot: true, noise: false, name: "unknown" })), "1.2.3.4", 1);
    expect(v.status).toBe("benign");
    expect(v.tags).toContain("business-service");
  });
  it("greynoise unknown + noise=false -> score 0", async () => {
    const v = await greynoiseVerdict(fake(200, JSON.stringify({ classification: "unknown", riot: false, noise: false })), "1.2.3.4", 1);
    expect(v.status).toBe("unknown");
  ...
  it("virustotal malicious ratio", async () => {
    ...
    const v = await virustotalVerdictIp(fake(200, body), "203.0.113.7", 1);
    expect(v.status).toBe("malicious");
    expect(v.score).toBe(9);
  });
  it("virustotal 404 -> notfound", async () => {
    const v = await virustotalVerdictIp(fake(404, ""), "1.2.3.4", 1);
    expect(v.status).toBe("notfound");
  });
  it("virustotal non-200/non-404 -> unavailable", async () => {
    const v = await virustotalVerdictIp(fake(403, "forbidden"), "1.2.3.4", 1);
    expect(v.status).toBe("unavailable");
  });
  it("virustotal bad JSON -> unavailable", async () => {
    const v = await virustotalVerdictIp(fake(200, "not-json"), "1.2.3.4", 1);
    expect(v.status).toBe("unavailable");
  });
  it("virustotal missing last_analysis_stats -> unknown", async () => {
    const v = await virustotalVerdictIp(fake(200, JSON.stringify({ data: { attributes: {} } })), "1.2.3.4", 1);
    expect(v.status).toBe("unknown");
  });
  it("virustotal all clean -> clean", async () => {
    ...
    const v = await virustotalVerdictIp(fake(200, body), "1.2.3.4", 1);
    expect(v.status).toBe("clean");
  });
