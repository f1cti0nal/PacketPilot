...
Implemented on branch `feat/saas-backend-foundation`; Supabase project ref `brkztcfhmrjjnbjzycie`. Migrations evolved from the planned 5 to **8** files (net-positive deviations), applied in this order and matching `list_migrations` exactly:
`0001_init` (schema), `0002_functions` (functions/triggers — split out from RLS for apply-order), `0003_rls` (policies), `0004_harden_functions` + `0004b_harden_functions_public` (revoke direct EXECUTE on the trigger fns from public/anon/authenticated; pin `set_updated_at` search_path — from the Task-4 review), `0005_views` (`admin_dashboard_stats`), `0006_bootstrap_admin`, `0007_perf_rls` (auth_rls_initplan subselect-wrap + split feature_flags FOR ALL + covering FK indexes — from the Task-8 advisor pass). `.env.local` is ignored via the pre-existing `*.local` glob (no `.gitignore` change needed). `seed.mjs` was replaced by SQL-only `supabase/seed.sql` (chosen seeding method). Final advisors: no ERROR-level; remaining WARNs justified (anon analytics ingestion by design; `is_admin` RPC-callable required by RLS; leaked-password protection deferred to the Phase-1 Auth setup). Carry-forward: tighten `analytics_insert_any` in Phase 7 (`user_id IS NULL` for anon + rate limit); enable leaked-password protection in Phase 1.
