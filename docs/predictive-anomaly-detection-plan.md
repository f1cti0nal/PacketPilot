...
Single-capture, egress-only, per-host. Deliberately deferred: a **cross-capture predictive mode**
(extend the BBL sidecar with a short per-metric trend and forecast the *next capture's* value,
flagging off-trend jumps — Holt on the per-capture series); **seasonality** (Holt-Winters / hour-of-day
profiles); **per-peer / per-port sub-series**; **ingress forecasting** for inbound-flood victims; and a
**forecast-band overlay** on the UI timeline chart (draw `forecast ± z·σ` behind the existing
`time_histogram`). Each is additive and does not disturb the shipped core.
